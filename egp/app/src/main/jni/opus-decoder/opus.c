...
Java_ai_sibylla_egp_client_decoder_OPUSDecoder_create(JNIEnv *env, jclass clazz, jint samplerate, jint bitdepth, jint channels, jint streams, jint coupled, jstring map)
{
    if(g_decoder)
    {
...
Java_ai_sibylla_egp_client_decoder_OPUSDecoder_decode(JNIEnv * env, jclass clazz, jbyteArray encoded, jint offset, jint length, jbyteArray pcm)
{
...
JNIEXPORT void JNICALL
Java_ai_sibylla_egp_client_decoder_OPUSDecoder_destroy(JNIEnv *env, jclass clazz) {
    if (g_decoder)
    {
        opus_multistream_decoder_destroy(g_decoder);
        g_decoder = NULL;
