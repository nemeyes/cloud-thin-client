...
public class SettupActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (TextUtils.isEmpty(address)) {
            address = ClientContext.ServerAddress;
        }
        mAddress.setText(address);
        String portNumber = Preferences.getServerPort();
        if (TextUtils.isEmpty(portNumber)) {
            portNumber = Integer.toString(ClientContext.ServerPortnumber);
        }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if(keyCode == ClientContext.REMOTECON_OK) {
    ...
    private boolean save() {
        ...
        ClientContext.ServerPortnumber = Integer.parseInt(portNumber);
        ...
        ClientContext.VideoBufferCapacity = videoBufferCapacity;
        ...
        ClientContext.VideoBufferSize = videoBufferSize * 1024;
        ...
        ClientContext.AudioBufferCapacity = audioBufferCapacity;
        ...
        ClientContext.AudioBufferSize = audioBufferSize * 1024;;

        Preferences.setAudioEnable(mAudioEnable.isChecked());
        ClientContext.AudioEnabled = mAudioEnable.isChecked();

        Preferences.setGyroEnable(mGyroEnable.isChecked());
        ClientContext.GyroEnabled = mGyroEnable.isChecked();

        Preferences.setControllerEnable(mControllerEnable.isChecked());
        ClientContext.ControllerEnabled = mControllerEnable.isChecked();
