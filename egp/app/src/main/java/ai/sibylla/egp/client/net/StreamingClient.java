...
public class StreamingClient extends LightweightClient {
    ...
    private boolean readCommand() {
        ...
        if(header == null) {
            Log.e(TAG, "readCommand : Header is null");
            mStreamingListener.onError(ClientContext.ERROR_CONNECTION_CLOSE);
            return false;
        }
        ...
        if(readBody(bodySize) < 0) {
            Log.e(TAG, "readCommand : bodySize is null");
            mStreamingListener.onError(ClientContext.ERROR_CONNECTION_CLOSE);
            return false;
        }
        ...
        switch(command) {
            ...
            case ClientContext.CMD_DESTROY_SESSION_INDICATION : // 1002
                break;

            case ClientContext.CMD_PLAY_RES: // 8012
				...
				if(mIsVideo && rcode != 0) {
					mStreamingListener.onError(ClientContext.ERROR_STREAM_REQ_FAIL);
					return false;
				}
                return true;
        }
    ...
    private void readStream() {
        ...
        while(mIsRun) {
            if(!readHeader(header)) {
                //Log.e(TAG, "readHeader is failed");
                mStreamingListener.onError(ClientContext.ERROR_CONNECTION_CLOSE);
                continue;
            }
            ...
            if(command !=1005 && command != 4003 && command != 4004 && command != 4005) {
                //Log.e(TAG, "readStream Command is "+command);
...
*/
    ...
    public void requestStream() {
        Header header = Header.newInstance(ClientContext.CMD_PLAY_REQ);
