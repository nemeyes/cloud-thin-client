...
public class MediaCodecHelper {
    ...
    static {
        ...
        // The Intel decoder on Lollipop on Nexus PlayerContext would increase latency badly
    ...
    public static boolean decoderSupportsAdaptivePlayback(MediaCodecInfo decoderInfo) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (isDecoderInList(blacklistedAdaptivePlaybackPrefixes, decoderInfo.getName())) {
                //LimeLog.info("VideoDecoder blacklisted for adaptive playback");
                return false;
            }
    ...
    public static String dumpDecoders() throws Exception {
        ...
        for (MediaCodecInfo codecInfo : getMediaCodecList()) {
            ...
            str += "VideoDecoder: "+codecInfo.getName()+"\n";
                    ...
                    if (requiredProfile != -1) {
                        for (CodecProfileLevel profile : caps.profileLevels) {
                            if (profile.profile == requiredProfile) {
                                //LimeLog.info("VideoDecoder " + codecInfo.getName() + " supports required profile");
                                return codecInfo;
                            }
                        }

                        //LimeLog.info("VideoDecoder " + codecInfo.getName() + " does NOT support required profile");
                    }
                    else {
                        return codecInfo;
                    }
